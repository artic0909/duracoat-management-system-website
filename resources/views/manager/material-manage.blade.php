...
<main class="main-content position-relative max-height-vh-100 h-100 mt-1 border-radius-lg">
                            ...
                            <div class="row g-2 material-row mb-2">
                                ...
                                <div class="col-md-2">
                                    <select name="paint_id[]" class="form-select select2-paint">
                                        ...
                                        <option value="{{ $paint->id }}">{{ $paint->ral_code }} -
                                            {{ $paint->brand_name }} - {{ $paint->shade_name}} - {{ $paint->finish }}
                                        </option>
                                        @endforeach
                                    </select>
                                </div>
    ...
    <div class="modal fade" id="viewModal{{$client->id}}" tabindex="-1" aria-labelledby="viewClientModalLabel"
        aria-hidden="true">
                        ...
                        <table class="table table-bordered align-items-center mb-0">
                            <thead class="bg-light">
                                <tr>
                                    ...
                                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Micron (Min - Max)
                                    </th>
                                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                        Paint Details</th>
                                </tr>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-sm">
                                        ...
                                        <span class="text-xs text-muted">Brand: {{ $paint->brand_name }} | Shade:
                                            {{ $paint->shade_name }} | Finish: {{ $paint->finish }}</span>
                                        @else
    ...
    <!-- Edit Modal -->
    @foreach ($clients as $client)
    <div class="modal fade" id="editModal{{$client->id}}" tabindex="-1" aria-labelledby="editClientModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-fullscreen">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <div class="modal-body">
                        ...
                        <div id="edit-material-details-container{{ $client->id }}">
                            ...
                            <div class="row g-2 material-row mb-2">
                                ...
                                <div class="col-md-2">
                                    <input type="text" name="material_name[]" class="form-control"
                                        value="{{ $material['material_name'] ?? '' }}" placeholder="Material Name"
                                        required>
                                </div>
                                <div class="col-md-1">
                                    <input type="text" name="quantity[]" class="form-control"
                                        value="{{ $material['quantity'] ?? '' }}" placeholder="Quantity" required>
                                </div>
                                <div class="col-md-1">
                                    <select name="unit[]" class="form-select" required>
                                        ...
                                        <option value="Meter" {{ $material['unit'] == 'Meter' ? 'selected' : '' }}>Meter
                                            (M)</option>
                                        <option value="FT" {{ $material['unit'] == 'FT' ? 'selected' : '' }}>Feet
                                            (FT)</option>
                                        <option value="INCH" {{ $material['unit'] == 'INCH' ? 'selected' : '' }}>Inch
                                        </option>
                                        <option value="SQFT" {{ $material['unit'] == 'SQFT' ? 'selected' : '' }}>Square
                                            Feet (SQFT)</option>
                                        <option value="SQM" {{ $material['unit'] == 'SQM' ? 'selected' : '' }}>Square
                                            Meter (SQM)</option>
                                    </select>
                                </div>
                                <div class="col-md-1">
                                ...
                                <div class="col-md-2">
                                    <select name="paint_id[]" class="form-select select2-paint">
                                        ...
                                        <option value="{{ $paint->id }}" {{ isset($material['paint_id']) && $material['paint_id'] == $paint->id ? 'selected' : '' }}>
                                            {{ $paint->ral_code }} - {{ $paint->brand_name }} - {{ $paint->shade_name}}
                                            - {{ $paint->finish }}
                                        </option>
                                        @endforeach
                                    </select>
    ...
    <!-- Delete Modal -->
    @foreach ($clients as $client)
    <div class="modal fade" id="deleteModal{{$client->id}}" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <form class="modal-content" action="{{ route('manager.client-material-manage.delete', $client->id) }}"
                method="POST">
                @csrf
                @method('DELETE')
